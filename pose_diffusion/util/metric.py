...
from pytorch3d.transforms.math import *
...
def so3_relative_angle_16(
    ...
    return so3_rotation_angle_16(R12, cos_angle=cos_angle, cos_bound=cos_bound, eps=eps)
...
def so3_rotation_angle_16(
    ...
    if cos_angle:
        ...
        if cos_bound > 0.0:
            bound = 1.0 - cos_bound
            return acos_linear_extrapolation_16(phi_cos, (-bound, bound))
        else:
            return torch.acos(phi_cos)
        
def acos_linear_extrapolation_16(
    x: torch.Tensor,
    ...
    acos_extrap[x_mid] = torch.acos(x[x_mid]).to(dtype=torch.float16)
...
def rotation_angle_16(rot_gt, rot_pred, batch_size=None):
    # rot_gt, rot_pred (B, 3, 3)
    rel_angle_cos = so3_relative_angle_16(rot_gt, rot_pred, eps=1e-4)
    rel_rangle_deg = rel_angle_cos * 180 / torch.pi
